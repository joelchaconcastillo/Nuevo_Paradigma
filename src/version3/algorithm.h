...
class CMOEAD
{
   ...
   public:
	...
	void updateTargetv2(CIndividual &target, CIndividual &trial);
...
void CMOEAD::evol_population()
{
   for(int i = 0; i < nOffspring; i++)
   {
    ...
    // updateTarget(pool[parent_idx[idx_target]], child);
      updateTargetv2(pool[parent_idx[idx_target]], child);
      eval_R2(child);
      eval_R2(pool[parent_idx[idx_target]]);
      if( child.fitness < pool[parent_idx[idx_target]].fitness){ pool[parent_idx[idx_target]] =  child; cout <<"YES"<<endl;}
...
{
   set<int> candidates,survivors, penalized; 
   priority_queue< pair<double, int> > pq;
   for(int i = 0; i < pool.size(); i++) candidates.insert(i);
   for(auto c_idx:candidates) pq.push(make_pair(-pool[c_idx].fitness, c_idx));

   while(survivors.size() < nPop && !pq.empty())
   {
	...
	bool flag = false;
        for(auto s_idx:survivors)
	{
	   min_dis = min(min_dis, distance_var(pool[s_idx].x_var, pool[idx].x_var));
           if( min_dis < D)
	   {
		flag=true;
		break;
	...
	//cout << survivors.size()<< "  "<<D <<endl;
   //farthes individuals..
   vector<double> distances(pool.size(), DBL_MAX);
   for(auto pdx:penalized)
	for(auto sdx:survivors)
	   distances[pdx] = min(distances[pdx], distance_var(pool[pdx].x_var, pool[sdx].x_var));
  while(survivors.size() < nPop)
  {
     pair<int, double> max_dcn = make_pair(-1, -1000000);
     for(auto pdx:penalized)
	if(max_dcn.second < distances[pdx]) max_dcn = make_pair(pdx, distances[pdx]);
     survivors.insert(max_dcn.first);
     penalized.erase(max_dcn.first); 
     for(auto pdx:penalized) distances[pdx] = min(distances[pdx], distance_var(pool[pdx].x_var, pool[max_dcn.first].x_var));
  }
...
//	int r = fronts.size()-1;
     for(int w = 0; w < nWeight; w++)
     {
       ...
       for(auto k:fronts[r])
       {
           ...
           //minv += fitnessfunction(indiv.y_obj[k], namda[w]);
       } 
       totalsum += minv;
     }
  }
...
void CMOEAD::updateTargetv2(CIndividual &target, CIndividual &trial)
{
  vector<double> x_var;
  vector<vector<double> > y_obj;
  for(auto x_value:target.x_var) x_var.push_back(x_value);
  ...
  for(auto y_vec:target.y_obj) y_obj.push_back(y_vec);
  for(auto y_vec:trial.y_obj) y_obj.push_back(y_vec);
  unordered_set<int> survivors, candidates, survivors_front, candidates_front;
  vector<set<pair<double, int> > > survivors_weight;
  for(int idx = 0; idx < y_obj.size(); idx++) candidates.insert(idx); 
  vector<vector<double> > table_fitness(nWeight, vector<double> (y_obj.size()));
  for(int w_id = 0; w_id < nWeight; w_id++)
  {
     for(int idx = 0; idx < y_obj.size(); idx++)
     {
        table_fitness[w_id][idx] = fitnessfunction(y_obj[idx], namda[w_id]);
     }
  }
     vector<int> Np(y_obj.size(), 0);
    vector<unordered_set<int> > Sp(y_obj.size());
   int rank = 0;
   for(int pidx1=0; pidx1< y_obj.size(); pidx1++)
   {
      for(int pidx2=0; pidx2< y_obj.size(); pidx2++)
